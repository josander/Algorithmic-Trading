...
money = zeros(length(prediction),length(training));
ratio = zeros(length(prediction),length(training));

n = 0;
j = 0;

for lengthLearningData = training;
    
    n = n + 1;
    
    disp(lengthLearningData)
    
    for lengthProg = prediction;
        ...
        ratio(j,n) = correct/(wrong+correct);
...
surf(ratio)
xlabel('Length learning data');
ylabel('Length prognosis');
zlabel('Ratio');

surf(money)
xlabel('Length learning data');
ylabel('Length prognosis');
zlabel('Ending capital')
