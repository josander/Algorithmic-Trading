...
dataLearn = xlsread('GOOG-LON_IGUS.xls');

% Get openinging price
openingLearn = dataLearn(:,2);

% Get closing price
closingLearn = dataLearn(:,5);

% Get price movement today and tomorrow
moveTodayLearn = closingLearn - openingLearn;
moveTomorrowLearn = moveTodayLearn(2:end);
...
seqLearn = getObservations(moveTodayLearn, closingLearn, delta);

% Get hidden sequenc e for learning
statesLearn = getHidden(moveTomorrowLearn, delta);
...
[trans, emis] = getModel(seqLearn(learningVec), statesLearn(learningVec));

% Get prognosis
[price, hidden] = getPrognosis(seq, startLearning, trans, emis, delta, closing);

buy = data(1:end-1,6);
sell = data(2:end,3);

% Calculate the return
endCapital = getEndingCapital(capital, buy, sell, startLearning, hidden);

%---------------------------- Validation ---------------------------------%
learningVec(end) = startLearning;
